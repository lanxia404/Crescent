...
DEFAULT_CFG = os.path.join(ROOT_DIR, "configs", "model", "tiny-char-256.yaml")
...
async def healthz():
    ...
    try:
        ensure_loaded(slot)
        return {"ok": True, "slot": slot, "device": str(_cached["device"])}
...
async def generate(req: GenReq):
    ...
    return GenResp(slot=slot, device=str(device), output=text)
